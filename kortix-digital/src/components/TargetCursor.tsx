...
export default function TargetCursor({
  ...
  useEffect(() => {
    ...
    const tickerFn = () => {
      if (!targetCornerPositionsRef.current || !cursorRef.current || !corners.length) return;
      const strength = activeStrengthRef.current;
      if (strength === 0) return;
      const cursorX = gsap.getProperty(cursorRef.current, "x") as number;
      const cursorY = gsap.getProperty(cursorRef.current, "y") as number;
      const cornerArr = Array.from(corners);
      cornerArr.forEach((corner, i) => {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      if (!target || !cursorRef.current || !corners.length) return;
      ...
      activeTarget = target;
      const cornerArr = Array.from(corners);
      cornerArr.forEach((corner) => gsap.killTweensOf(corner, "x,y"));
      gsap.killTweensOf(cursor, "rotation");
      spinTl.current?.pause();
      gsap.set(cursor, { rotation: 0 });

      if (cursorColorOnTarget) {
        gsap.to(cornerArr, { borderColor: cursorColorOnTarget, duration: 0.15, ease: "power2.out" });
      ...
      const leaveHandler = () => {
        ...
        if (cursorColorOnTarget && corners.length) {
          gsap.to(Array.from(corners), { borderColor: cursorColor, duration: 0.15, ease: "power2.out" });
    ...
    >
      ...
      {[0, 1, 2, 3].map((i) => {
        ...
        return (
          <div
            key={i}
            className={`target-cursor-corner absolute top-1/2 left-1/2 w-3 h-3 border-[3px] ${borders}`}
